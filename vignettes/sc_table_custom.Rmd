...
db_schema$`Other Classifications`$`Country of origin`
```
...
Omitting certain classification elements from the query is possible with the `recodes` parameter of the `/table` endpoint.
Currently, `sc_table_custom()` does not provide support for recodes.
