...
## Bulding a Custom Table Step by Step {#step-by-step}
...
This will request only the mandirory fields and default measures for that database.
...
For classifications with more levels of hierarchies, more valuesets will be present.
...
sc_table_custom(population, count)
```
...
- **performance** Traffic between the client and server is reduced which might 
  lead to consierably faster API responses.
...
- **cell limits (equest)** If a single request would contain more than 1 million 
  cells, a [cell count error](sc_last_error.html#CELL_COUNT) is thrown.

### Filtering Data {#filter}

As an example for fiitering data, we can request a table from the tourism
...
In this case, the cell values for Gemany and the Netherlands are just added
to calculate the etries for Arrivals and Nights spent.
However, in other cases STATcubeR might decide it is more appropriate
...
classifications. As an example, let's look at the tourism acivity in the
...
or Nights spent in either of these three cities durng that month.
...
If you get spurious warnings or have suggestions on how these typechecks might
be improved, please issue a feature request to the STATcubeR bugtracker.
