...
Again, the contents do have a tree structure which is similar to the sidebar of the "Table View" in the GUI.
...
See the [SuperSTAR API documentation](https://docs.wingarc.com.au/superstar/9.12/open-data-api/open-data-api-reference/schema-endpoint) for a complete list of schema types.
...
```{r}
my_db_info$Factors$id
group_info <- sc_schema(my_db_info$Factors$id)
group_info
```
...
```{r}
group_info$`Average hours usually worked per week`$id
```

Alternatively, use the `depth` parameter of `sc_schema`.
This will make sure that the enties of the tree are returned up to the `"valueset"` level.
