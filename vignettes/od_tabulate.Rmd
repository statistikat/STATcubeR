---
title: "Tabulating STATcbeR Datasets"
description: >
...
The first part will use the cancer dataset from the [OGD overview article](od_table.html).
After that, other features of `$tabulate()` will be demonstrated with the data from the structure of earnings survey (SES).
...
`{STATcubeR}` uses `rowsum()` to ensure a good performance with big datasets.
...
STATcubeR will use `pmatch()` to match the supplied strings with the metadata
...
We can just remove the toal code before running `od_tabulate()`.
...
available to define the tabulation. If you want to use `STATcubeR`
